...
export default function ModAuditoria(){
  ...
  async function auditarRepo(){
    ...
    if(!r.ok){ setRepoRep('⛗ No se pudo leer (HTTP '+r.status+'). Si el repo es PRIVADO, pega un token arriba.'); return; }
    const j=await r.json();
    const paths=(j.tree||[]).map(n=>n.path);
    const L=['AUDITORÍA DE REPO · '+REPO,'---','MUERTOS (deben estar ELIMINADOS):'];
    DEAD.forEach(d=>{ const present=paths.some(p=>p===d||p.startsWith(d+'/')); L.push((present?'⛗ PRESENTE: ':'✔ ELIMINADO: ')+d); });
  ...
  async function sembrar(){
    ...
    if(await cnt('product_families')===0){ await supabase.from('product_families').insert([{code:'FP001',name:'BICICLETA',active:true},{code:'FP002',name:'BICICLETA ELECTRICA',active:true},{code:'FP003',name:'MAQUINA',active:true},{code:'FP004',name:'SCOOTER ELECTRICO',active:true},{code:'FP005',name:'ACCESORIO',active:true}]); L.push('✅ familias +5'); }
  ...
  return (<div style={S.card}>
    ...
    {tab==='datos'? <div>
      ...
      <p style={{...S.sub,marginBottom:10}}>Lee el árbol de GitHub y evidencia la limpieza. Si el repo es privado, pega un token.</p>
