...
function BarsV(props){
  var data=props.data||[]; var color=props.color||T.brand; var max=1;
...
function Donut(props){
  var a=Number(props.a)||0; var b=Number(props.b)||0; var t=(a+b)||1; var p=Math.round(a/t*100);
  return (
...
export default function ModKpis(){
  ...
  useEffect(function(){ (async function(){
    var r1=await supabase.from('work_orders').select('*').limit(3000);
  ...
  function stdHoras(o){
    var k=K(o); var sv=String(k.tipo_servicio||'').toUpperCase(); var eq=String(k.tipo_equipo||'').toUpperCase();
  ...
  var unidades=ots.reduce(function(s,o){ return s+(Number(o.cantidad_unidades)||0); },0);
  ...
  var entSi=cerr.filter(function(o){ return String(K(o).entrega_tiempo||o.entrega_tiempo).toUpperCase()==='SI'; }).length;
  var entNo=cerr.filter(function(o){ return String(K(o).entrega_tiempo||o.entrega_tiempo).toUpperCase()==='NO'; }).length;
  var pctEntrega=(entSi+entNo)>0?Math.round(entSi/(entSi+entNo)*100):0;
  var diasArr=cerr.map(function(o){ return Number(K(o).dias!=null?K(o).dias:o.dias_reparacion)||0; }).filter(function(n){ return n>0; });
  var avgDias=diasArr.length?Math.round(diasArr.reduce(function(a,b){return a+b;},0)/diasArr.length*10)/10:0;
  ...
  ots.forEach(function(o){
    var k=(o.created_at||'').slice(0,7); if(!k) return;
  ...
  meses.forEach(function(m){ dataOTs.push({l:m.slice(5),v:porMes[m].o}); dataVenta.push({l:m.slice(5),v:Math.round(porMes[m].v/1000)}); });

  var porEq={};
  ots.forEach(function(o){ var k=K(o).tipo_equipo||'—'; porEq[k]=(porEq[k]||0)+(Number(o.cantidad_unidades)||1); });
  var dataEq=Object.keys(porEq).map(function(k){ return {l:k.slice(0,12),v:porEq[k]}; });

  var porTec={};
  ots.forEach(function(o){
    var t=o.tecnico_nombre||''; if(!t) return;
    if(!porTec[t]) porTec[t]={u:0,h:0};
    porTec[t].u+=Number(o.cantidad_unidades)||1; porTec[t].h+=stdHoras(o);
  });
  var dataTecU=Object.keys(porTec).map(function(k){ return {l:k.split(' ')[0],v:porTec[k].u}; });
  var dataTecH=Object.keys(porTec).map(function(k){ return {l:k.split(' ')[0],v:Math.round(porTec[k].h*10)/10}; });
  ...
  return (
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:12,marginBottom:14}}>
        ...
        <div style={S.card}><div style={S.sub}>Unidades</div><div style={{fontSize:22,fontWeight:800}}>{unidades}</div></div>
        <div style={S.card}><div style={S.sub}>Cerradas / Abiertas</div><div style={{fontSize:22,fontWeight:800}}>{cerr.length} / {abiert.length}</div></div>
        ...
        <div style={S.card}><div style={S.sub}>Entrega a tiempo</div><div style={{fontSize:22,fontWeight:800,color:pctEntrega>=90?T.ok:T.warn}}>{pctEntrega}%</div></div>
        <div style={S.card}><div style={S.sub}>Días reparación (prom)</div><div style={{fontSize:22,fontWeight:800}}>{avgDias}</div></div>
      ...
      {ots.length===0? <div style={S.card}><p style={S.sub}>Aún no hay OTs cargadas. Cuando importes tu historial (ANALISIS → Importar Datos), aquí verás los mismos números de tu Excel: 153 OTs · 260 unidades · FTF 92,81% · SLA ALTA 142.</p></div> : null}
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:14}}>
        ...
        <div style={S.card}><h2 style={S.h2}>Horas por técnico</h2><BarsV color={T.teal} data={dataTecH}/></div>
      </div>
    </div>
  );
}
