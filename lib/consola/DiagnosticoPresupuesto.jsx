...
export default function DiagnosticoPresupuesto(props){
  ...
  async function guardarPres(){     var payload={ot_id:ot.id,customer_id:ot.customer_id,items:items.filter(function(x){return x.concepto;}),total:total,vigencia:vig||null}; if(pres) await supabase.from('presupuestos').update(payload).eq('id',pres.id); else await supabase.from('presupuestos').insert([payload]); avisar('✅ Presupuesto guardado',T.ok); onChanged(); var r=await supabase.from('presupuestos').select('*').eq('ot_id',ot.id).limit(1); setPres((r.data||[])[0]||null); }
  ...
  return (
    <div style={{background:T.surface2,borderRadius:10,padding:12,marginBottom:12}}>
      ...
      <h3 style={{...S.h2,margin:'12px 0 6px'}}>💰 Presupuesto (nace del diagnóstico)</h3>
