...
export default function Solicitud(){
  ...
  const [f,setF]=useState({nombre:'',rut:'',telefono:'',email:'',region:'',comuna:'',direccion:'',tipo_producto:'Bicicleta',marca_modelo:'',serial:'',garantia:'No, garantía vencida',falla:'Ruido / traqueteo',descripcion:''});

  useEffect(()=>{
    supabase.from('regions').select('*').then(({data})=>setRegiones(data||[]));
    supabase.from('comunas').select('*').then(({data})=>setComunas(data||[]));
  },[]);
  const comunasDeRegion = comunas.filter(c=>c.region_id===Number(f.region));
  ...
  async function enviar(e){
    ...
    const {data,error:err}=await supabase.rpc('crear_ot_publica',{
      ...
      p_region_codigo:f.region, p_direccion:(f.comuna?f.comuna+' · ':'')+f.direccion,
      p_tipo_producto:f.tipo_producto, p_marca_modelo:f.marca_modelo, p_serial:f.serial,
      p_garantia:f.garantia, p_falla:f.falla, p_descripcion:f.descripcion, p_fotos:fotos
    });
    setEnviando(false);
      ...
      <div style={{maxWidth:640,margin:'0 auto'}}>
        ...
        {otNum ? (
          <div style={{background:'#e9f6ee',border:'1px solid #b8dfc6',borderRadius:10,padding:26,textAlign:'center'}}>
            ...
            <p style={{fontSize:12.5,color:'#5c636b',lineHeight:1.6}}>Guarda este número único. Haz <b>seguimiento en línea</b> y genera insistencias en <b>torque-os-self.vercel.app/seguimiento</b> con este número y tu email. Te contactaremos por teléfono o WhatsApp para coordinar.</p>
          </div>
        ) : (
          <form onSubmit={enviar} style={{background:'#fff',border:'1px solid #e2ddd2',borderRadius:10,padding:22}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
              ...
              <div><label style={lab}>Región *</label><select style={inp} required value={f.region} onChange={e=>setF({...f,region:e.target.value,comuna:''})}><option value="">Elegir…</option>{regiones.map(r=><option key={r.id} value={r.codigo}>{r.nombre}</option>)}</select></div>
              <div><label style={lab}>Comuna *</label><select style={inp} required value={f.comuna} onChange={e=>setF({...f,comuna:e.target.value})} disabled={!f.region}><option value="">{f.region?'Elegir…':'Primero elige región'}</option>{comunasDeRegion.map(c=><option key={c.id} value={c.nombre}>{c.nombre}</option>)}</select></div>
              <div style={{gridColumn:'1 / -1'}}><label style={lab}>Dirección (calle y número)</label><input style={inp} value={f.direccion} onChange={e=>setF({...f,direccion:e.target.value})} /></div>
            ...
            <button type="submit" disabled={enviando||subiendo} style={{width:'100%',padding:13,borderRadius:8,border:0,background:enviando?'#c9c3b6':'#ff6b2c',color:'#14100c',fontWeight:700,fontSize:14,cursor:enviando?'wait':'pointer'}}>
