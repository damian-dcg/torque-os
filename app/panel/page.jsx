...
function FormCliente({ regiones, onOk }){
  ...
  async function guardar(e){
    ...
    const {error}=await supabase.from('customers').insert([{
      nombre:f.nombre, rut:f.rut||null, tipo:f.tipo,
      region_id:f.region_id?Number(f.region_id):null,
      telefono:f.telefono||null, email:f.email||null
    }]);
    if(error){ setMsg('Error: '+error.message); }
    else { setMsg(''); setF({nombre:'',rut:'',tipo:'final',region_id:'',telefono:'',email:''}); onOk('Cliente creado: '+f.nombre); }
  }
  return (
...
function FormOT({ customers, sats, regiones, onOk }){
  ...
  async function guardar(e){
    ...
    const {data,error}=await supabase.from('work_orders').insert([{
      customer_id:Number(f.customer_id), tipo:f.tipo, prioridad:f.prioridad,
      region_id:f.region_id?Number(f.region_id):null,
      asignado_company_id:f.asignado?Number(f.asignado):null,
      descripcion:f.descripcion||null, canal:'interno'
    }]).select();
    if(error){ setMsg('Error: '+error.message); }
    else { setMsg(''); setF({customer_id:'',tipo:'servicio',prioridad:'media',region_id:'',asignado:'',descripcion:''}); onOk('OT-'+data[0].ot_number+' creada'); }
  }
  return (
    <form onSubmit={guardar} style={{background:C.panel,border:`1px solid ${C.borde}`,borderRadius:10,padding:16,marginBottom:16}}>
...
export default function Panel(){
  ...
  return (
    <main style={{minHeight:'100vh',background:C.fondo,color:C.tinta,fontFamily:'system-ui,sans-serif'}}>
      ...
      <nav style={{display:'flex',gap:8,padding:'16px 22px 0'}}>
      ...
      <section style={{padding:'16px 22px'}}>
        {tab==='panel' && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
            <div style={{background:C.panel,border:`1px solid ${C.borde}`,borderTop:`3px solid ${C.naranja}`,borderRadius:10,padding:16}}>
              <div style={etiqueta}>OTs activas</div>
              <div style={{fontSize:30,fontFamily:'monospace'}}>{ots.filter(o=>o.estado!=='cerrada'&&o.estado!=='anulada').length}</div>
            </div>
            <div style={{background:C.panel,border:`1px solid ${C.borde}`,borderTop:`3px solid ${C.teal}`,borderRadius:10,padding:16}}>
              <div style={etiqueta}>Clientes</div>
              <div style={{fontSize:30,fontFamily:'monospace'}}>{customers.length}</div>
            </div>
            <div style={{background:C.panel,border:`1px solid ${C.borde}`,borderTop:`3px solid ${C.amarillo}`,borderRadius:10,padding:16}}>
              <div style={etiqueta}>SAT registrados</div>
              <div style={{fontSize:30,fontFamily:'monospace'}}>{sats.length}</div>
            </div>
            <div style={{background:C.panel,border:`1px solid ${C.borde}`,borderTop:`3px solid ${C.verde}`,borderRadius:10,padding:16}}>
              <div style={etiqueta}>Próxima OT (secuencia única)</div>
              <div style={{fontSize:30,fontFamily:'monospace',color:C.verde}}>OT-{proxOT}</div>
            </div>
          </div>
        )}
            ...
            <div style={{background:C.panel,border:`1px solid ${C.borde}`,borderRadius:10,overflow:'hidden'}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                ...
                <tbody>{customers.map(c=>(
                  <tr key={c.id}><td style={td}>{c.nombre}</td><td style={td}>{c.tipo}</td><td style={td}>{c.rut||'—'}</td><td style={td}>{c.telefono||'—'}</td></tr>
                ))}</tbody>
              </table>
              {customers.length===0 && <p style={{padding:14,color:C.gris,fontSize:12.5}}>Aún no hay clientes. Crea el primero arriba.</p>}
            </div>
          </div>
        )}
        ...
        {tab==='ots' && (
          <div>
            ...
            <div style={{background:C.panel,border:`1px solid ${C.borde}`,borderRadius:10,overflow:'hidden'}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr><th style={th}>OT</th><th style={th}>Cliente</th><th style={th}>Tipo</th><th style={th}>Estado</th><th style={th}>Prioridad</th></tr></thead>
                <tbody>{ots.map(o=>(
                  <tr key={o.id}>
                    ...
                    <td style={{...td,color:colorEstado(o.estado)}}>{o.estado}</td>
                    <td style={td}>{o.prioridad}</td>
                  </tr>
                ))}</tbody>
              </table>
              {ots.length===0 && <p style={{padding:14,color:C.gris,fontSize:12.5}}>Aún no hay OTs. La primera será la OT-5001.</p>}
            </div>
          </div>
        )}
      </section>
