...
const NAV=[
 ['OPERACIÓN',[['kpis','Dashboard KPIs'],['ots','Órdenes de Trabajo'],['buzon','Buzón del Agente'],['agenda','Agenda'],['nueva','Nueva OT']]],
 ['CATÁLOGOS',[['maestros','Maestros y Parámetros'],['productos','Productos y Garantías'],['checklists','Checklists'],['clientes','Clientes'],['activos','Activos/Equipos'],['bodega','Bodega/Repuestos']]],
 ['DINERO',[['presupuestos','Presupuestos'],['red','Red SAT y Liquidaciones'],['bonos','Bonos']]],
 ['SISTEMA',[['importar','Importar Datos'],['conectores','Conectores'],['config','Configuración']]]
];

export default function Consola(){
  const [me,setMe]=useState(null); const [tenant,setTenant]=useState(null);
  const [tab,setTab]=useState('kpis'); const [toast,setToast]=useState(null);
  ...
  const buzón=(ots.length? []:[]);
  const visibles=ots.filter(o=>{const t=q.toLowerCase(); return (!t||String(o.ot_number).includes(t)||((cust[o.customer_id]||{}).nombre||'').toLowerCase().includes(t))&&(!fEst||o.estado===fEst);});

  return (
    <main style={S.main}>
      {toast&&<div style={S.toast(toast.c)}>{toast.t}</div>}
      <div style={{display:'flex',minHeight:'100vh'}}>
        <aside style={{width:230,background:T.surface,borderRight:`1px solid ${T.border}`,padding:'16px 12px',flexShrink:0}}>
          <h1 style={{...S.h1,marginBottom:4}}>TORQUE<span style={{color:brand}}>·OS</span></h1>
          <p style={{...S.sub,marginBottom:16}}>{tenant?tenant.nombre:''}</p>
          {NAV.map(([g,items])=>(<div key={g} style={{marginBottom:14}}>
            <div style={{...S.sub,fontWeight:800,marginBottom:6,letterSpacing:.5}}>{g}</div>
            {items.map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={{display:'block',width:'100%',textAlign:'left',padding:'8px 10px',borderRadius:8,border:0,marginBottom:2,background:tab===k?brand:'transparent',color:tab===k?'#fff':T.text,fontWeight:tab===k?700:500,fontSize:13,cursor:'pointer'}}>{l}</button>)}
          </div>))}
          <a href="/tecnico" style={{...S.btnO(T.info),textDecoration:'none',display:'block',textAlign:'center'}}>Vista Técnico</a>
          <button onClick={async()=>{await supabase.auth.signOut(); router.replace('/');}} style={{...S.btnO(T.danger)}}>Salir</button>
        </aside>
        <div style={{flex:1,minWidth:0}}>
          <div style={S.wrap}>
            ...
            {tab==='ots'&&(
              <div>
                <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap'}}>
                  ...
                  <select style={{...S.input,flex:1,minWidth:160,marginBottom:0}} value={fEst} onChange={e=>setFEst(e.target.value)}><option value="">Todos</option>{['Ingresada','Asignada','En Ruta','Llegada','Trabajando','Esperando Repuesto','Revisión QA','Cerrada','Rechazada'].map(s=><option key={s}>{s}</option>)}</select>
                </div>
                <div style={S.card}><table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{visibles.slice(0,100).map(o=>(<tr key={o.id} onClick={()=>setSel(o)} style={{cursor:'pointer'}}>
            ...
            {tab==='buzon'&&<div style={S.card}><h2 style={S.h2}>Buzón</h2><p style={S.sub}>Rechazos, repuestos y pausas aparecen aquí en tiempo real.</p></div>}
        ...
        <div style={S.modal} onClick={()=>setSel(null)}>
          <div style={S.modalCard} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:10}}>
              <button style={{...S.btnO(T.info),width:'auto'}} onClick={()=>{setTab('clientes');setSel(null);}}>Ver cliente</button>
              <button style={{...S.btnO(T.teal),width:'auto'}} onClick={()=>{setTab('activos');setSel(null);}}>Ver activos</button>
              <button style={{...S.btnO(T.warn),width:'auto'}} onClick={()=>{setTab('bodega');setSel(null);}}>Bodega</button>
            </div>
