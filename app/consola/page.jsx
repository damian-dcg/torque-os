...
export default function Consola(){
  ...
  const [users,setUsers]=useState([]); const [sats,setSats]=useState([]); const [avail,setAvail]=useState([]);
  ...
  async function cargar(uid){
    const [m,ten,o,c,ev,i,n,b,ck,u,s,av]=await Promise.all([
  ...
  async function cambiarEstado(ot,estado){
    ...
    if(error) alert(error.message); else { setSel(null); cargar(me.auth_uid); }
  }
  ...
  async function guardarAvail(e){
    ...
    if(error) alert(error.message); else { setAvailForm({user_id:'',desde:'',hasta:'',motivo:''}); cargar(me.auth_uid); }
  }
  ...
  const visibles=ots.filter(o=>{
    ...
    const okE=!fEst||o.estado===fEst;
    return okQ&&okE;
  });

  const TABS=[['dash','Dashboard'],['buzon','Buzón ('+buzón.length+')'],['ots','Órdenes'],['agenda','Agenda']];

  return (
    <main style={S.main}>
      <header style={{position:'sticky',top:0,zIndex:20,background:T.surface,borderBottom:`1px solid ${T.border}`,padding:'12px 16px',display:'flex',gap:12,alignItems:'center',flexWrap:'wrap'}}>
        <h1 style={{...S.h1,color:T.text}}>TORQUE<span style={{color:brand}}>·OS</span></h1>
        <span style={S.sub}>{tenant?tenant.nombre:''}</span>
        <nav style={{display:'flex',gap:6,marginLeft:'auto',flexWrap:'wrap'}}>
          {TABS.map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={{padding:'8px 14px',borderRadius:10,border:tab===k?'0':`1px solid ${T.border}`,background:tab===k?brand:'transparent',color:tab===k?'#0B1220':T.muted,fontWeight:700,fontSize:13,cursor:'pointer'}}>{l}</button>)}
          <a href="/tecnico" style={{padding:'8px 14px',borderRadius:10,border:`1px solid ${T.border}`,color:T.muted,fontSize:13,textDecoration:'none'}}>Vista técnico</a>
          <button onClick={async()=>{await supabase.auth.signOut(); router.replace('/');}} style={{padding:'8px 14px',borderRadius:10,border:`1px solid ${T.danger}`,background:'transparent',color:T.danger,fontSize:13,cursor:'pointer'}}>Salir</button>
        </nav>
      </header>
      <div style={S.wrap}>
        ...
        {tab==='dash'&&(
          <div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:12,marginBottom:14}}>
              {[['OTs activas',activas.length,T.info],['En Revisión QA',ots.filter(o=>o.estado==='Revisión QA').length,T.violet],['Esperando repuesto',ots.filter(o=>o.estado==='Esperando Repuesto').length,T.danger],['Rechazadas (ruta)',ots.filter(o=>o.estado==='Rechazada').length,T.danger],['Insistencias clientes',ins.length,T.warn],['NPS promedio',npsProm,T.ok]].map(([l,v,c],i)=>(
                <div key={i} style={{...S.card,marginBottom:0,borderTop:`3px solid ${c}`}}>
                  <div style={S.sub}>{l}</div>
                  <div style={{fontSize:28,fontWeight:800,color:T.text}}>{v}</div>
                </div>))}
            </div>
            <div style={S.card}>
              <h2 style={S.h2}>Últimas OTs</h2>
        ...
        {tab==='buzon'&&(
          <div style={S.card}>
            <h2 style={S.h2}>Buzón del agente · rechazos, repuestos y pausas</h2>
            ...
            {buzón.map(e=>(
              <div key={e.id} style={{border:`1px solid ${T.border}`,borderRadius:10,padding:12,marginBottom:10,background:T.surface2}}>
                <div style={{display:'flex',justifyContent:'space-between',gap:8,flexWrap:'wrap'}}>
                  <b style={{color:e.evento==='motivo'?T.danger:T.warn,fontSize:14}}>{e.evento==='motivo'?'RECHAZO CON MOTIVO':e.evento==='alerta_repuesto'?'REPUESTO SOLICITADO':'PAUSA REPUESTO'} · OT-{(ots.find(o=>o.id===e.ot_id)||{}).ot_number||e.ot_id}</b>
                  <span style={S.sub}>{fmtFecha(e.created_at)}</span>
                </div>
        ...
        {tab==='agenda'&&(
          <div>
            <div style={S.card}>
              <h2 style={S.h2}>Registrar indisponibilidad de técnico</h2>
              <form onSubmit={guardarAvail} style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:8}}>
                <select style={S.input} required value={availForm.user_id} onChange={e=>setAvailForm({...availForm,user_id:e.target.value})}>
            ...
            <div style={S.card}>
              <h2 style={S.h2}>Indisponibilidades</h2>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr><th style={S.th}>Técnico</th><th style={S.th}>Desde</th><th style={S.th}>Hasta</th><th style={S.th}>Motivo</th></tr></thead>
                <tbody>{avail.map(a=>(
                  <tr key={a.id}><td style={S.td}>{(users.find(u=>u.id===a.user_id)||{}).nombre||a.user_id}</td><td style={S.td}>{a.desde}</td><td style={S.td}>{a.hasta}</td><td style={S.td}>{a.motivo||'—'}</td></tr>))}</tbody>
              </table>
            </div>
          </div>
        )}
        ...
        <div style={S.modal} onClick={()=>setSel(null)}>
          <div style={{...S.modalCard,maxWidth:760,maxHeight:'88vh',overflow:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
              ...
              <span style={S.pill(estColor(sel.estado))}>{sel.estado}</span>
            </div>
              ...
              <div style={{...S.card,background:T.surface2}}>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  <select style={{...S.input,flex:1,marginBottom:0}} value={asig.tipo+':'+asig.id} onChange={e=>{ const [t,i]=(e.target.value||':').split(':'); setAsig({tipo:t,id:i}); }}>
                    ...
                    <optgroup label="Taller central / agentes">{users.map(u=><option key={'t'+u.id} value={'tec:'+u.id}>{u.nombre}</option>)}</optgroup>
            ...
            {sel.estado==='Revisión QA'&&(
              <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:10}}>
                ...
                <button style={{...S.btnO(T.warn),flex:1}} onClick={()=>cambiarEstado(sel,'Trabajando')}>↩ Devolver a Trabajando</button>
              </div>)}
            ...
            {Object.entries(sel.checklist_responses||{}).filter(([k])=>k.startsWith('w_')).length>0&&(
            ...
            {sel.financial_data&&sel.financial_data.totalCost!=null&&(
              <p style={{color:T.ok,fontWeight:800,marginTop:10}}>Total terreno: {fmtCLP(sel.financial_data.totalCost)} · base {fmtCLP(sel.financial_data.baseCost)}</p>)}
            ...
            {events.filter(e=>e.ot_id===sel.id).map(e=><p key={e.id} style={{color:T.sub,fontSize:12,margin:'3px 0'}}>{fmtFecha(e.created_at)} · {e.evento} {e.detalle?('· '+JSON.stringify(e.detalle).slice(0,120)):''}</p>)}
          </div>
        </div>
      )}
    </main>
